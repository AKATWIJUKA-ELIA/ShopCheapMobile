...
export default function ChatScreen() {
      ...
      if (res.ok) {
        ...
        if (conversationData && conversationData.messages) {
          ...
          setMessages(prev => {
            ...
            return [...remoteMessages, ...pendingLocal];
          });
        } else {
  ...
  const renderMessageRow = ({ item, index }: { item: ChatMessage; index: number }) => {
    ...
    >
      ...
      {loading && messages.length === 0 ? (
        ...
        <Animated.FlatList
          ref={flatListRef}
          data={messages}
          keyExtractor={(item, index) => item._id || `local-${index}-${item._creationTime}`}
